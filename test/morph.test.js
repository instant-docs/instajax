...
describe("Morph html", () => {
    ...
    test("should morph a simple html string", () => {
        ...
        expect(window.document.body.innerHTML).toBe(newHtmlString);
    })
    ...
    test("morph whole html", async () => {
        ...
        expect(normalize(window.document.documentElement.outerHTML)).toBe(normalize(newHtmlString));
    });

    test("morph complex html", async () => {
        const oldHtmlString = readFileSync(join(__dirname, "old.html"), "utf-8");
        const newHtmlString = readFileSync(join(__dirname, "new.html"), "utf-8");
    ...
    test("morph complex html 2", async () => {
        const oldHtmlString = readFileSync(join(__dirname, "new.html"), "utf-8");
        const newHtmlString = readFileSync(join(__dirname, "old.html"), "utf-8");
        const dom = new JSDOM(oldHtmlString);
        window = dom.window;
        document = dom.window.document;
        morphDOM(newHtmlString);
